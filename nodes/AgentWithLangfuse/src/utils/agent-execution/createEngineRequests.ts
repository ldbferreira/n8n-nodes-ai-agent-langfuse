...
function debugHitl(label: string, payload: Record<string, unknown>) {
	if (process.env.N8N_AI_AGENT_DEBUG_HITL !== 'true') return;
	try {
		console.debug(`[HITL:createEngineRequests] ${label}`, JSON.stringify(payload));
	} catch {
		console.debug(`[HITL:createEngineRequests] ${label}`, payload);
	}
}
...
export function createEngineRequests(
	...
	return toolCalls
		.map((toolCall) => {
			...
			if (hitlMetadata) {
				debugHitl('before-hitl-input-expand', {
					toolName: toolCall.tool,
					toolCallId: toolCall.toolCallId,
					nodeName,
					originalToolInput: toolInput,
					hitlParameters: (input as IDataObject).hitlParameters,
					hitlMetadata,
				});
				...
				debugHitl('after-hitl-input-expand', {
					toolName: toolCall.tool,
					toolCallId: toolCall.toolCallId,
					nodeName,
					finalActionInput: input,
					hitlMetadata,
				});
			}
