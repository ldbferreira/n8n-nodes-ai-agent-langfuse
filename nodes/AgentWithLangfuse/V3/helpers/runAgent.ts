...
} from '../../src/utils/agent-execution';
import { buildResponseMetadata } from '../../src/utils/agent-execution/buildResponseMetadata';
import { buildTracingMetadata, getTracingConfig } from '../../src/utils/tracing';
import type {
	EngineRequest,
	EngineResponse,
	IExecuteFunctions,
...
import type { CallbackHandler } from 'langfuse-langchain'; 		// add langfuse handler 
...
export async function runAgent(
	...
	langfuseHandler?: CallbackHandler, 		// add langfuse handler 
	...
	const executeOptions = {
		...
		signal: ctx.getExecutionCancelSignal(), callbacks: langfuseHandler ? [langfuseHandler] : [],
	};
	...
	if (
		...
		if (result.toolCalls && result.toolCalls.length > 0) {
			...
			ctx.logger.debug('ToolsAgent V3 generated streaming tool actions', {
				itemIndex,
		...
		ctx.logger.debug('ToolsAgent V3 generated non-streaming tool actions', {
			itemIndex,
			actions: actions.map((action) => ({
				nodeName: action.nodeName,
				id: action.id,
